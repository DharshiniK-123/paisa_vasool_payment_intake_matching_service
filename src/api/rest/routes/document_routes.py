...
from sqlalchemy import select, and_, func as sqlfunc, text
...
async def save_records(document_id: int,body: SaveRecordsRequest,db: AsyncSession = Depends(get_db),user: dict = Depends(get_current_user),):
    try:
            ...
            if body.document_type == "PAYMENT":
                ...
                if invoice_no and payment_reference:
                    ...
                    if existing:
                        ...
                        print(f"No dupe found for ref={payment_reference}")
            ...
            if body.document_type == "INVOICE":
                ...
                if invoice_number:
                    ...
                    if existing:
                        ...
                        print(f"No dupe found for invoice_number={invoice_number}")
        ...
        raise HTTPException(
            status_code=409,
            detail=f"Duplicate record detected: {str(e.orig)}",
        )
    except Exception as e:
...
async def get_document_payments(document_id: int,db: AsyncSession = Depends(get_db),user: dict = Depends(get_current_user),):
    try:
        result = await db.execute(
            ...
            .where(
                PaymentDetail.document_id == document_id,
                PaymentDetail.is_deleted == False,
            )
        )
