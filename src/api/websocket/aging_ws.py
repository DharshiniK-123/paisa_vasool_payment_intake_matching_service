...
async def aging_run_ws(websocket: WebSocket):
    ...
    try:
        async with AsyncSessionLocal() as db:
                ...
                try:
                    ...
                    if reminder is None:
                        ...
                        await websocket.send_json({
                            ...
                            "message":  f"{invoice.invoice_number} — skipped (reminder sent recently)",
                        })
                    else:
                        generated += 1
                        await websocket.send_json({
                            ...
                            "message":  f"{invoice.invoice_number} → {item['config'].severity} — reminder sent ✓",
                        })
            ...
            await websocket.send_json({
                "type":    "done",
                "message": f"Aging job complete — sent: {generated}, skipped: {skipped}, failed: {failed}",
                "sent":    generated,
                "skipped": skipped,
                "failed":  failed,
            })
